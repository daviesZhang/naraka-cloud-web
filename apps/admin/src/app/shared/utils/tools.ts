 ...
 * @param format string
 */
export function dateToString(date: Date, format: string = 'yyyy-MM-dd HH:mm:ss'): string {
 ...
 * @param removeEmpty 是否移除空参数
 */
export function transformParams(params: { [key: string]: any } = {}, timestamp: string | true = 'yyyy-MM-dd HH:mm:ss', removeEmpty = true): {} {
  params = {...params};
  Object.values(params).filter(value => !Array.isArray(value) && typeof value === 'object' && Object.keys(value).length >= 1)
    .forEach(value => (Object.assign({}, params, value)));
  // @ts-ignore
  return Object.keys(params).map(key => {
    let value = params[key];
    if (!Array.isArray(value) && typeof value === 'object' && Object.keys(value).length >= 1) {
      return null;
    }
    if (removeEmpty) {
 ...
 */
export function getTreeData<T extends { [key: string]: any }>(data: Array<T>,
                                                              params: {
                                                                children?: string,
 ...
 * @param params
 */
export function changeDataToGridTree<T extends {}>(data: Array<T>, params: {
  parent: (item: T) => string | number,
  id: (item: T) => string | number
}): Array<T> {
...
export function arrayPartition<T>(items: Array<T>, size = 3): Array<Array<T>> {
  ...
  return items.reduce((previousValue, currentValue) => {
    !previousValue.length || previousValue[previousValue.length - 1].length === size ? previousValue.push([currentValue]) : previousValue[previousValue.length - 1].push(currentValue);
 ...
 */
export function sum(data: Array<{ [key: string]: any }>, keys: Array<string>, currency = true): { [key: string]: number | string | null } {
